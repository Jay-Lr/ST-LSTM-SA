...
def adam_updates(params, cost_or_grads, lr=0.001, mom1=0.9, mom2=0.999):
  ...
  if not isinstance(cost_or_grads, list):
    ...
    grads = cost_or_grads
  t = tf.Variable(1., 'adam_t')   # adam_t = 1.0
  ...
  def __init__(self, configs):
    ...
    self.x = [       
        tf.placeholder(tf.float32, [
            self.configs.batch_size, self.configs.total_length,           # (8,20,64,64,1)
    ...
    self.real_input_flag = tf.placeholder(tf.float32, [                # (8,9,64,64,1)
    ...
    for i in range(self.configs.n_gpu):
      with tf.device('/gpu:%d' % i):
        with tf.variable_scope(  
          ...
          gen_ims = output_list[0]   # (8,10,64,64,1)
          loss = output_list[1]
          loss_train.append(loss / self.configs.batch_size)
          # gradients
          all_params = tf.trainable_variables() 
          grads.append(tf.gradients(loss, all_params))  # tf.gradient(y,x)
